...
```{r echo=FALSE, fig.align='center', fig.cap="KNN with k=20", fig.pos='h', fig.width=75, fig.height=75}
...
distribution of school distances: the majority of them has a distance below the chilometer,
...
💡 Measures of spatial autocorrelation describe the degree to which observations at spatial locations are similar to each other.
...
p-value, we can confirm that the columns with the highest statistics are:
  ...
  * Population under 20 over Total Population,
  * Students over Population under 20 (as stated before, despite it is not evident in the aggregated results).
...
By applying the step function, the model is simplified to students, schools and mean of students per school. However, the adjusted r-squared is too low
...
Also in this case, the p-value is above $0.05$, suggesting no spatial autocorrelation in the residuals. 
We can have a look at the plot to see how residuals are distributed all over the municipalities. 
...
definition. For instance, Palù del Fersina and Sagron Mis often switch from LL to LH quadrant. Also, since this feature is
a proportion that goes from $0.08$ to $0.19$, many municipalities
...
**Porte di Rendena**, **Comano Terme** and **Dro**, which are usually in the same HH quadrant and have a high proportion of students.
...
**Primiero San Martino di Castrozza** and **Sagron Mis** share a boundary, but their proportion
of students is $0.15$ and $0.10$ respectively, enhancing a great dissimilarity between these two municipalities.
...
based on the different concepts of neighbourhood. It seems like $18$ or
...
The following two plots show:
  ...
  * the territories where the Local Moran's values are higher (darker
    ...
    Trentino (e.g. Primiero San Martino di Castrozza, Castello Tesino,
    Sagron Mis etc), but also around the most populated areas (e.g. Riva del Garda and territories around Rovereto etc).
...
while the critical cut-off and knn found significance in the eastern part (e.g.
...
test of spatial autocorrelation in OLS residuals, in particular considering the Population under 20 over the total population model.
...
significance. Also, the overall p-value is above $0.05$, therefore no
...
Same principle applies to SAR, which has an even higher p-value than SDM,
indicating the absence of statistical significance. The relevance of
predictors remains unaltered, despite _students_ and _students over the young population_ are boundary situations (slightly above $0.05$).
...
about the predictors (except for the _mean students per school_).
...
The same happens for the SEM, except for the high relevance of the predictor "_mean of students per school_".
