...
# for index in list(schools.index)[:5]:
#     # Getting the school coordinates
#     place = schools.loc[index, 'geometry']
#     # Get the graph
#     G = get_graph(place, 'walk')
#     isochrone_polys = []
#     center_node = get_center_node(G, place)
#     for trip_time in sorted(trip_times, reverse=True):
#         subgraph = nx.ego_graph(
...
#     # Saving polygons on dictionary
#     schools_polies[index] = isochrone_polys
# # %%
# # Creating a first map with POIs and polygons
# schools_polies[0] = schools_polies[0].to_json()
# #%%
   
# m = folium.Map((schools.loc[0,'geometry'].y,schools.loc[0,'geometry'].x),zoom_start=12)

# # Adding isochrones polygons
# poly_fg = folium.FeatureGroup(name="Isochrones")
# poly_fg.add_to(m)
# for i in range(4):
#     folium.GeoJson(schools_polies[0] , 
#                    style_function=style).add_to(poly_fg)
